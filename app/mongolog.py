...
def dns_get_subdomain(subdomain, time):
    ...
    try:
        if time != None:
            find['date'] = {
                '$gte': datetime.datetime.fromtimestamp(int(time))
            }
    except:
        pass
...
def dns_delete_request(_id, subdomain):
    collection.update_one({'uid':subdomain, '_id':ObjectId(_id)}, {'$set':{'_deleted':True}})
...
def http_delete_request(_id, subdomain):
    http.update_one({'_id':ObjectId(_id),'uid':subdomain}, {'$set':{'_deleted':True}})
