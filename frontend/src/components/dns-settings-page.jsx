...
export const DnsSettingsPage = ({ dnsRecords: propDnsRecords = [], fetched = false, user, toast }) => {
  ...
  const add = (domain = "", type = 0, value = "") => {
    const newDnsRecords = [...dnsRecords, { domain, type, value, subdomain: user.subdomain }];
  ...
  const saveChanges = useCallback(() => {
    const filteredRecords = dnsRecords.filter((value) => value.domain.length > 0 && value.value.length > 0);
  ...
  return (
    <div className="card card-w-title card-body">
      <div className="grid">
        ...
        <div className="col-6">
          <Button label="Save changes" icon="pi pi-save" className="p-button-success p-button-text" style={{ float: "right" }} onClick={saveChanges} />
        </div>
      </div>
      <div className="grid">
        <div className="col-12">
          <p>You can use % to select a random IP from multiple (e.g. 127.0.0.1%8.8.8.8)</p>
        </div>
      </div>
      <div className="grid">
        <div className="col-12">
          <Button label="Add record" onClick={add} icon="pi pi-plus" className="p-button-text" style={{ float: "right" }} />
        </div>
      </div>
      {renderedDnsRecords}
    </div>
