...
export class HeaderInput extends Component {
    ...
    filterHeader = (event) => {
        if (this.props.headersData) {
            let results = this.props.headersData.filter((header) => {
        ...
        return (
            <div className="grid">
                <div className={this.isDesktop() ? "col-6" : "col-12"}>
                    <div className="grid">
                        <div className="col-5">
                            <AutoComplete width={"100%"} delay={0} minLength={1} placeholder="Header-Name" size={30} suggestions={this.state.filteredHeaders}
                                completeMethod={this.filterHeader} value={this.state.header} onChange={event => this.changeEvent(event, true)} />
                        </div>
                        <div className="col-5">
